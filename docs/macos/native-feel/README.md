...
Agent  ❯ Before architecture, the decision tree
         ...
         several common shapes — quick check first:
           ...
           - Memory floor?       (<150MB → build native instead; see `references/05-memory-truths.md`)
...
- **"Is my designer’s spec ‘native enough’?"** → the 53-item conventions audit in `references/06-native-conventions.md`.
...
- Not for apps with strict <150 MB memory budgets or <100 ms cold-start budgets (macOS floor is ~90 MB, Windows ~130 MB, before app code).
...
│   ├── 06-native-conventions.md            # 53 items the native-feel audit checks
...
本模块是 **WebView 跨平台** native-feel skill(技术栈不同于纯原生 macOS)。其中可复用的**原生交互/窗口约定**已抽进区块树:[`../swiftui/native-conventions.md`](../swiftui/native-conventions.md) 与 [`../window/window-conventions.md`](../window/window-conventions.md)(并注明与原生 SwiftUI 做法的适用场景差异)。总入口见 [`../index.md`](../index.md)。
