...
- **Cold start must be < 100 ms (or < 50 ms for instant-popup style):**
  → **Don't use this architecture.** The WebView + Node boot floor is ~200–300 ms minimum. See `references/02-architecture.md` for detail. Build native.
...
- **Under 150 MB resident:**
  → **Don't use this architecture.** The floor is ~90 MB on macOS and ~130 MB on Windows for an empty WebView + Node (before your app code). Build native.
