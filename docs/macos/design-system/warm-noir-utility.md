...
> Follow the Warm Noir Utility style from `docs/macos/design-system/warm-noir-utility.md`: black floating macOS HUD surfaces, large notch-like rounded panels, restrained warm orange state accents, SF Pro-like system typography, compact utility layout, subtle borders, soft glow only for active state, and no marketing-style decorative UI.
