...
- **Full-screen `--flash` capture** — the completion flash is a transient full-screen `.screen`-blended edge glow (§5.10), so the per-tab rect crop can't frame it. `--flash` (`UITestMode.flash`) makes it static and reproducible: `AppDelegate` (1) builds `CompletionFlashWindowController` even under uitest (normally gated off), (2) lays a `makeUITestFlashBackdrop(on:)` dark-gradient `NSWindow` at `.statusBar + 7` — **below** the glow/panel (`.statusBar + 8`) — so the additive `.screen` glow pops instead of washing out on a bright wallpaper and the user's other windows stay hidden behind it (no need to `osascript`-hide apps), and (3) calls `CompletionFlashService.holdForUITest(names:)`, a uitest-only entry that pins `flashLevel = 1` + shows the toast with **no** auto-reset/cooldown so the glow doesn't decay before capture. The `--flash` path deliberately **skips `notchOpen`** so the notch stays collapsed — matching the real scenario (coding with the notch tucked away, AI finishes → edge flash + toast) and letting the toast read clearly instead of being buried under an expanded panel; the toast/HUD overlay in `NotchView` renders regardless of open state. It also seeds a **minimal** state via `UITestSeeder.seedFlash(aiStore:)` (only a working Claude session — no media/calendar/agents/pomodoro) so the collapsed notch's single `CompactBadgesView` primary badge is the Claude Code crab + spinner, not a higher-priority pomodoro/agent badge. `scripts/uitest-flash-screenshot.sh` launches `--uitest --tab=claude --flash`, waits, runs a whole-display `screencapture -x` (not the rect crop), `sips`-downscales to width 1600, and restores the install. Output → `docs/images/completion-flash.png`.
