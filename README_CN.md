...
- **AI 用量配额** —— 在 AI 标签页以卡片展示 Claude Code、Codex 与 Gemini 的用量配额（使用率 % + 重置倒计时），数据来自各 CLI 的 OAuth 凭证。检测到 Codex / Gemini CLI 已登录时自动显示对应段。zcode 用量来自 ZCode 官方使用的 BigModel 配额接口（5 小时 + 每周 token 窗口，使用率 % 与重置倒计时），凭据通过解密 CLI 本地凭据文件获得；不可用时回退为从 CLI 本地数据库（`~/.zcode/cli/db/db.sqlite`）统计的实际用量（今日 / 近 7 天）。
- **AI 状态悬浮胶囊** — 任意 AI 会话运行中或等待你操作时，屏幕右上角（默认）持续显示一个可拖动的悬浮胶囊，兼作 herdr 风格的状态灯：黄色脉冲 = 运行中，红色 = 等待权限选择，实心绿 = 等待输入，中空绿 = 全部完成（数秒后淡出）。点击展开为会话列表 + 详情面板（模型、Token、上下文进度、当前工具）。详情页标题栏的按钮可一键跳回该会话所在的终端/IDE：hook 脚本上报 CLI 进程 pid，NemoNotch 在事件到达时沿进程树上溯到宿主 GUI 应用并缓存，pid 失效或被复用时回退为按 bundle id 激活。tmux/ssh 下或来自插件型 emitter（opencode）的会话没有宿主，不显示按钮。
