...
- **AI usage quota** — shows your Claude Code, Codex, and Gemini usage quotas (utilization % + reset countdown) as a card in the AI tab, read from each CLI's OAuth credential. The Codex and Gemini sections appear automatically when those CLIs are signed in. zcode usage comes from the same BigModel quota API ZCode's own usage page uses (5-hour + weekly token windows with % and reset countdown), authenticated by decrypting the CLI's local credential file; if that's unavailable it falls back to local usage stats (today / last 7 days) from the CLI's sqlite database.
- **AI Status Capsule** — a draggable floating capsule (top-right of the main screen by default) stays visible while any AI session is running or waiting on you, doubling as a herdr-style status light: yellow pulsing dot = running, red = waiting for a permission choice, solid green = waiting for your input, hollow green = all done (fades a few seconds after everything goes quiet). Click to expand into a session list + detail panel (model, tokens, context progress, current tool). The detail header's button jumps straight to the terminal/IDE hosting the selected session: the hook script reports the CLI's pid, NemoNotch walks the process tree to the GUI app at event time, and activation falls back to bundle-id when the pid is gone or recycled. Sessions under tmux/ssh or from plugin-based emitters (opencode) have no host and show no button.
- **Lock Screen AI Panel** — while your Mac is locked, a compact dark card under the lock-screen clock lists the AI sessions still running (Claude Code / Gemini / opencode / zcode): project name, live status (running / compacting / awaiting approval), elapsed time and context %. Approval-needing sessions sort first and are tinted. Purely a glance display — it never takes clicks. Toggleable in Settings.
