...
enum OpencodePluginInstaller {
    ...
    static func pluginSource(port: UInt16) -> String {
        ...
        // Shared auth token: the server rejects /hook POSTs without it (v2).
        let hookToken = ""
        try { hookToken = readFileSync(homedir() + "/.NemoNotch/hook-token", "utf8").trim() } catch (_) {}

        const post = (body) => {
          try {
            fetch(URL_BASE + "/hook", {
              method: "POST",
              headers: { "Content-Type": "application/json", "X-NemoNotch-Token": hookToken },
