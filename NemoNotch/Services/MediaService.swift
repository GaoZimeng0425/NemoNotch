...
final class MediaService {
    ...
    private func updateNowPlaying() {
        // Try the legacy MRMediaRemoteGetNowPlayingInfo first (fast path on macOS < 15.4).
        // On macOS 15.4+ this often returns empty for non-Apple apps, so fall back to
        // the new MRNowPlayingController ObjC API.
        remote.getNowPlayingInfo { [weak self] info in
            guard let self else { return }
            if let info, !info.isEmpty, !MediaService.isInfoEmptyMetadata(info) {
                self.applyInfo(info)
            } else {
                self.remote.queryViaNewControllerAPI { fallback in
                    self.applyInfo(fallback)
                }
            }
        }
    }
